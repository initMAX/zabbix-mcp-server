...
## v2.0.0 - in progress (target: 2026-06)

Plugin-host major release. Major version bump because the source tree splits into a generic MCP **host** (admin portal, OAuth, audit pipeline, /metrics, reporting engine) and a per-plugin tree starting with the bundled Zabbix integration under `plugins/zabbix/zabbix_mcp/`. External integrations (Nagios, PRTG, Atlassian, FastSpring, ...) plug in via `plugin.json` manifests and the new plugin loader. Other 2.0 headliners: SSO via LDAP / Active Directory and SAML 2.0 (issue #46), tool-level audit log with HMAC tamper chain + redactor (issue #49), conservative OAuth DCR profile by default (refuses wildcard scope at registration), SIEM / syslog forwarder (RFC 5424 / CEF / LEEF / JSON over UDP / TCP / TLS), Prometheus `/metrics` endpoint, `/health/full` aggregate health, the `auditor` role on the admin portal, multi-arch Docker images, and the OAuth consent rebuild with per-plugin / per-tool grants. The OAuth client revoke 500 (#48) and `verify_ssl = false` legacy renegotiation (#51) fixes from v1.31 are forward-merged.

Plugin loader discovery half (`zabbix_mcp.plugin_loader.discover()`) ships in v2.0; the subprocess + JSON-RPC runtime half stays in flight for a 2.x point release once a second bundled plugin exists to test against. Migration is wheel-name compatible (`pip install zabbix-mcp-server` still works, import path stays `zabbix_mcp`).

### Plugin-readiness + brand thread (carried over from the abandoned v1.31 line)

The original v1.31 plan was a plugin-readiness + brand-evolution release. v2.0 absorbs all that work plus the heavier SSO / audit / metrics additions, hence the major version jump. **Brand**: the admin portal UI rebrands from "Zabbix MCP" to "initMAX MCP" (the repo, package name, CLI binary, MCP server identity, and `pip install zabbix-mcp-server` all stay unchanged - purely a UI label). **Layout**: the bundled Zabbix module's source tree moves under `plugins/zabbix/zabbix_mcp/` so every MCP module lives at one canonical path; the sidebar reorganises into three sections (Authentication & Clients / MCP Modules / System); a new `/modules` page lists installed modules + the "Coming soon" plugin roadmap (Nagios / PRTG, NetBox, Atlassian Jira / Confluence, FastSpring); the bundled module's `plugin.json` manifest, a reference example plugin (`examples/example-plugin/`), and a stub `docs/PLUGIN-DEVELOPMENT.md` ship so external plugin authors can read against a stable contract. **OAuth consent rewrite**: the consent screen is rebuilt with per-plugin sections, per-tool grants (236 tools), a Read-only / Read+Write toggle that cascades to per-tool checkboxes, and a search filter; the underlying scope grammar gains `tool:<name>` and `plugin:<id>.read` / `plugin:<id>.write` forms while keeping legacy `monitoring` / `alerts` / etc. working unchanged. Plus the OAuth Clients revoke HTTP 500 fix (#48), an upstream `/revoke` patch so public PKCE clients can revoke their own tokens (RFC 7009 §2.1), and several smaller polish items (login `*` chip replaced with a sentence, in-portal modal for revoke confirm, NetBox added to the roadmap list, ...).

### Added

- **Enterprise audit log admin panel** - Settings -> Audit log mirrors the Zabbix admin-panel layout (Enable audit logging / Log background server events / Enable internal housekeeping / Data storage period / Max file size / Reset defaults). Master toggle is honoured at every audit write site; the toggle change itself is always audited (action `audit.toggle`) so a reviewer can see when audit was disabled. Disabling requires typing `DISABLE` in a confirmation field with copy citing ISO 27001 / SOC 2 / NIS2. Persistent danger banner appears on every page until audit is re-enabled.
- **Time-based audit retention + daily rotation** - housekeeping daemon rotates the live `audit.log` to `audit.log.YYYY-MM-DD.gz` (gzip) at end-of-day or when the file exceeds `max_file_size_bytes`, then purges archives older than `data_storage_period`. Default 31 days, configurable as a Zabbix-style time period (`31d`, `90d`, `1y`, ...). `0` disables time-based purge for operators who pipe everything to an external SIEM.
- **`auditor` admin role** - read-only role scoped to `/audit` and the dashboard. An `_AuditorRoleMiddleware` bounces any other URL to `/audit` (303). Separation of duties: SOC / compliance reviewers read the audit trail without seeing token prefixes, OAuth client metadata, server config, or any other admin-sensitive surface.
- **External SIEM / syslog forwarder** ([`audit_forwarder.py`](plugins/zabbix/zabbix_mcp/admin/audit_forwarder.py)) - background daemon ships every audit row to an external SOC / SIEM destination. Eleven protocols across four wire formats: RFC 5424 syslog (UDP / TCP / TLS), Common Event Format / CEF (UDP / TCP / TLS, for ArcSight / Splunk / Sentinel), Log Event Extended Format / LEEF (UDP / TCP / TLS, for QRadar), JSON over TCP / TLS for ELK / Graylog / generic HTTP receivers. TLS uses `ssl.create_default_context()` with optional `ca_cert` PEM for private CAs. Reconnect with exponential backoff (1s, 2s, 4s, ... capped at 60s). TCP/TLS framing is octet-counted per RFC 6587 §3.4.1. Bounded in-memory queue (default 10,000 rows) drops oldest under SOC backpressure, recording a `messages_dropped_queue_full` counter. Settings -> External SIEM forwarding form persists destination + protocol; live status indicator shows connection state, queue depth, sent / failed / dropped counters, last error.
- **Tool-level audit logging** ([#49](https://github.com/initMAX/zabbix-mcp-server/issues/49) Track A, credit [@musaabhasan](https://github.com/musaabhasan) via discussion [#42](https://github.com/initMAX/zabbix-mcp-server/discussions/42)) - every MCP tool invocation now writes one structured audit row to `/var/log/zabbix-mcp/audit.log`. The schema is **fixed and bounded**: each row carries `oauth_subject`, `mapped_zabbix_user`, `mcp_session_id`, `tool_name`, `scopes`, `policy_decision` (`allow` / `deny_scope` / `deny_read_only` / `deny_token_invalid` / `deny_token_expired` / `deny_server` / `deny_ip` / `deny_other` / `error`), `denial_reason`, a bounded `target` object (resource handles only - `hostid`, `groupid`, `itemid`, `eventid`, ...), a bounded `filters` object (`severities`, `monitored`, `active_only`, ...), `result_count` bucketed as `0` / `1` / `"N"`, and `ip`. An incident reviewer can answer "did anyone read trigger XYZ in the last 24h?" with a single grep instead of reconstructing from MCP session logs. Hooked at every exit path of `_make_tool_handler` (236 wrapped Zabbix tools) and at every exit path of all 13 extension tools (`graph_render`, `anomaly_detect`, `capacity_forecast`, `item_threshold_search`, `problem_active_get`, `host_status_get`, `hostgroup_overview_get`, `infrastructure_summary_get`, `item_history_summary_get`, `report_generate`, `action_prepare`, `action_confirm`, `zabbix_raw_api_call`) - allow, deny, and error sites all emit. Schema documented in [`docs/AUDIT.md`](docs/AUDIT.md).
- **Client-side audit log** at `/var/log/zabbix-mcp/client-audit.log` - a redacted-twice copy of every `tool.invoke` row, safe for an operator to share with the connected AI client (Claude Desktop, ChatGPT, an automation script). The client stream drops operator-internal fields (`oauth_subject`, `mapped_zabbix_user`, `mcp_session_id`, full `denial_reason`, `scopes`, `ip`, `filters`) and keeps only the AI-client perspective: what tool was called, was it allowed, how many results came back, what bucket the denial fell into.
- **`audit_self_get` MCP tool** (issue #49 Phase 5) - in-memory ring buffer (max 100 entries per subject) of recent client-audit rows, exposed as a self-service activity feed. A connected client pulls its own recent invocations to answer "what did I just do?" without the operator having to share the log file. Cross-client isolation enforced server-side - each subject sees only its own ring. Reset on server restart; bounded so a misbehaving client cannot grow the process RSS.
- **Centralised audit redactor** ([`audit_redactor.py`](plugins/zabbix/zabbix_mcp/admin/audit_redactor.py)) - single boundary that strips credentials, tokens, and other secrets from every audit-bound payload at `write_audit` / `write_tool_audit`. Substring-match denylist covers passwords, API keys, OAuth tokens, refresh tokens, code verifiers, PKCE verifiers, CSRF tokens, session cookies, MFA / TOTP secrets, HMAC signatures + nonces, Zabbix-specific credentials (TLS-PSK identity + secret, SNMP community strings, LDAP bind passwords, SMTP / webhook secrets, ODBC connection strings). Hash-bearing keys (`token_hash`, `password_hash`, `client_secret_hash`, ...) bypass the denylist with prefix-only retention for correlation. Long-string truncation at 512 chars keeps the log readable when an LLM accidentally pastes a large blob.
- **Bounded audit extractors** ([`audit_extractors.py`](plugins/zabbix/zabbix_mcp/audit_extractors.py)) - first-pass scrub that splits a tool's kwargs into `target` and `filters` buckets, dropping anything not in the allow-list of resource-reference / filter keys. A denied call's audit row gets only the resource references the caller targeted, never the full kwargs. Defence-in-depth: the redactor's denylist is a second pass on whatever survives the extractor.
- **Conservative OAuth DCR profile** (issue #49 Track B) - `[oauth].dcr_profile = "conservative"` is the default for `POST /register` (RFC 7591 dynamic client registration). The profile rejects wildcard scope grants at registration time (`scope = "*"` returns `invalid_client_metadata`), enforces exact-string redirect URI matching at `/authorize`, and short-circuits a few other footguns. Operators who need the v1.30 behaviour can opt back in with `dcr_profile = "permissive"`.
- **OAuth consent screen wildcard warning banner** - the existing "Grant full access" checkbox now reveals a danger-styled banner explaining the blast radius (every tool on every server, including writes that mutate Zabbix configuration). The banner appears on tick + disappears on untick. Existing wildcard-requested-by-client banner is unchanged - this is the **operator-side warning** when they consider granting it.
- **OAuth Clients "Last activity" column** - the `/oauth-clients` page gains a column showing the most recent tool invocation per client_id, derived from a 512 KB tail-scan of the audit log on every render. Clients that registered but never invoked a tool show as "never" - prime revocation candidates. A `last_tool_invoke_at` field also lives on `TokenInfo` for legacy bearer tokens.
- **Per-token Zabbix user mapping** - new optional `[tokens.<id>] zbx_user = "Admin"` config field surfaces as `mapped_zabbix_user` in every audit row that token writes. Lets an incident reviewer correlate an MCP tool invocation back to a Zabbix-side auditlog entry written by the same underlying API user. Empty when not configured (the audit row carries `null` rather than guessing).
- **Role parity documentation** at [`docs/ROLES.md`](docs/ROLES.md) - matrix of which admin portal role can do what, reverse-engineered from the `session.role` checks across `plugins/zabbix/zabbix_mcp/admin/views/*.py`. Calls out by-design asymmetries (operators can create tokens but only admins can delete them; operators can edit report templates but only admins can delete them) so future role gates are reviewed against an explicit contract.
- **Audit log negative test contract** at [`tests/test_audit_negatives.py`](tests/test_audit_negatives.py) - five regression tests cover the deny-side guarantees from issue #49: scope deny gets logged with the right policy_decision; severity-bypass attempts via raw `problem_get` land in `filters` so a reviewer sees what was actually requested; expired tokens fail closed with `deny_token_invalid`; two calls inside the same MCP session correlate via `oauth_subject + mcp_session_id`; denied requests carry `target.<resource_id>` but **not** raw kwargs (extractor drops + redactor scrubs as defence-in-depth).
- **OAuth consent screen rebuild** (`oauth_consent.html` + `oauth_login.py`) - the previous consent screen exposed only six tool-group checkboxes (`monitoring`, `alerts`, ...). v1.31 redesigns it per-plugin and per-tool: each plugin renders as its own card with a logo, read / write tool counts, and a Read-only / Read + Write radio. An expandable "Pick individual tools" panel lists every tool the plugin exposes (236 for the bundled Zabbix module) with a one-line description, write badges on mutating tools, group sub-headers, and a free-text filter. Picking "Read-only" cascades to dim + un-tick the per-tool write rows so the operator gets a clear visual that write tools are not part of a read-only grant. Picking "Read + Write" or "None" releases them. A wildcard "Grant full access" row stays at the top for admin operators who want one-click full grant.
- **Scope grammar extension** for issued tokens. `[tokens.X].scopes` (and OAuth-issued access tokens) now accept three new forms in addition to the legacy bundled tool groups (`monitoring`, `alerts`, `users`, `data_collection`, `administration`, `extensions`):
  - `tool:<canonical_name>` - explicit per-tool grant (e.g. `tool:host_get`, `tool:problem_acknowledge`).
  - `plugin:<id>.read` - read tools from one plugin (e.g. `plugin:zabbix.read`).
  - `plugin:<id>.write` - read + write tools from one plugin (subsumes `.read`).
  - `plugin:<id>` (no suffix) - same as `.write`.
  Tokens issued before v1.31 with legacy scopes work unchanged - the `_filter_tools_by_token` filter and `check_token_authorization` runtime check both fall through to the v1.30 prefix-match path when none of the new forms apply. The audit log entry for `oauth.consent_granted` ships a `granted_scopes_summary` object (`{wildcard, plugins, tools_count, tools_sample, legacy}`) that summarises the grant in a fixed-size shape regardless of how many per-tool grants were ticked.
- **Admin portal sidebar restructure** - three section labels (Authentication & Clients / MCP Modules / System) replace the previous flat list. Each section holds related entries: OAuth Clients + Tokens + Admin Users under Authentication; a collapsible Zabbix module group (Servers, Templates, Reports, History) under MCP Modules; Settings + Audit Log + Update Notifications under System. Operators familiar with v1.30 keep the same set of pages - none are renamed, none are removed - just grouped.
- **Collapsible Zabbix module group** in the sidebar with its own SVG logo (red Z monogram, centered, on a rounded square), a green status dot indicating the module is enabled, and a chevron to expand / collapse the submenu (Servers, Templates, Reports). Expand / collapse state is persisted per-browser in `localStorage`. This is the shape future plugins will follow: each plugin gets its own group entry with its own logo, status dot, and submenu of plugin-provided pages.
- **`/modules` admin page** listing every MCP module currently registered with the server, plus a "Coming soon" roadmap (Nagios / PRTG, NetBox, Atlassian Jira / Confluence, FastSpring, ...) and a "+ Install plugin..." button that opens a modal pointing at issue #47, SECURITY.md, and the example-plugin reference. Action buttons on the bundled Zabbix row (Configure servers / Templates / Disable) cross-link to existing pages; Disable is intentionally inert in v1.31 because the loader is not yet shipping.
- **Dashboard "Active MCP modules" tile** replaces the v1.30 "Report Templates" stat. The "Zabbix Servers" tile is renamed to "Zabbix Instances" and gains the same module-zabbix.svg icon so it visually belongs to the Zabbix plugin row.
- **`plugins/zabbix/plugin.json`** - bundled module manifest at the canonical plugin location. Documentary in v1.31 (the host implements the Zabbix integration directly), forward-compatible with the upcoming loader. Plugin authors can read it as a real-world example.
- **`examples/example-plugin/`** - reference implementation: a ~120-line standalone stdio MCP server with a single `example__echo` tool, plus `plugin.json`, `pyproject.toml`, and a README explaining the contract. The plugin is not loaded by the host in v1.31 (loader is forthcoming) but plugin authors can clone it and run `python -m example_plugin` to validate their understanding of the stdio JSON-RPC contract today.
- **`docs/PLUGIN-DEVELOPMENT.md`** - placeholder how-to that points at SECURITY.md (trust model), `config.example.toml` (operator-side schema), `examples/example-plugin/`, and issue #47. Expands into a full plugin author guide when the loader ships.
- **`install.sh add-plugin / remove-plugin / update-plugin / list-plugins`** subcommand stubs - operators who try `./install.sh add-plugin nagios` get a clear "loader is in development" message pointing at the contract design docs, instead of "Unknown argument". The actual install runtime ships with the loader release.
- **Brand: admin portal UI rebrand to "initMAX MCP"** - the sidebar header, login screen, OAuth login + consent + error screens, and HTML page titles read "initMAX MCP" instead of "Zabbix MCP". The repo, package on PyPI, the `zabbix-mcp-server` CLI binary, the systemd unit, the OAuth issuer URL, the MCP server name returned in `initialize`, and existing audit logs are deliberately unchanged - existing operator muscle memory and existing AI-client connectors keep working without reconfiguration.
- **MCP server `instructions=` string mentions plugin extensibility** so models like Claude / GPT understand the host can grow tool sets at runtime when the loader ships. Examples cited: `nagios__service_status`, `jira__issue_create`, `fastspring__order_get`. No tool list change in v1.31.
- **NetBox added to "Coming soon"** plugin list (between Nagios/PRTG and Atlassian) - source-of-truth IPAM / DCIM, resolves devices and IP addresses to Zabbix hosts, cross-references rack / site / tenant context for problems.

### Fixed

- **OAuth Clients "Revoke" returned HTTP 500** (#48) - `oauth_clients.py` accessed `session.username` on the admin portal session; the actual attribute is `session.user`. The error masked the revoke action so the `[oauth_clients.<id>]` config row stayed in `config.toml` and the operator's only workaround was to SSH in and edit the file manually + restart. Live-tested against an actual ChatGPT custom apps client on student-postgresql-01: revoke now succeeds, the audit log records `oauth_client.revoke`, in-memory access + refresh tokens for that client are invalidated, and the next request from the client receives 401.
- **`/revoke` (RFC 7009) rejected public PKCE clients** with `client_secret: Field required`. Upstream `mcp.server.auth.handlers.revoke.RevocationRequest` declares `client_secret: str | None` but without a default value, so Pydantic v2 treated it as required. v1.31 patches the schema at server startup (`_patch_fastmcp_revocation_request_optional_secret`) so the field defaults to `None` - public PKCE-only clients (Claude Desktop, mcp-inspector, mcp-remote) can now revoke their own tokens with just `token` + `client_id` per RFC 7009 §2.1. Confidential clients still have to send their secret; the `client_authenticator` (called before the schema validation) enforces that for them.
- **Revoke confirm dialog used the native browser confirm popup** instead of the in-portal modal that the rest of the admin portal uses (Restart, Delete token, etc.). Replaced with a `showConfirm()` modal. The implementation uses data-attributes (`data-form-id`, `data-client-name`) + `addEventListener` rather than inline `onclick` because the Jinja `tojson` filter inside an `onclick=` attribute hits HTML-quote-escaping issues when the client_id contains non-ASCII characters.
- **OAuth login screen rendered the wildcard scope as a literal `*` chip** alongside legitimate scope chips, which read as a UI glitch to operators. v1.31 detects the wildcard-only request and replaces the chip list with a sentence ("Requested access: all available MCP tools. You will choose which tool groups to actually grant on the next screen.") so the meaning is unambiguous. The disclaimer below the form was rewritten from "By signing in you grant the scopes listed above for one session" to "After signing in, the next screen lets you choose which specific tools and modules this client may use" to match the new two-step consent flow.
- **`/modules` action buttons rendered as unstyled blue underline links** on first ship - they referenced a non-existent `.btn-secondary` class. Replaced with `.btn-ghost` and wrapped in a flex container.

### Changed

- **`[server].tool_prefix` config option** - new opt-in field that prefixes every bundled Zabbix tool name (e.g. `zabbix__host_get` instead of `host_get`). Default empty preserves v1.30 byte-identical naming. Useful when running this host side-by-side with other MCP modules so tools cannot collide. Validated at boot (`^[a-z][a-z0-9_]*$`); invalid values raise a clear `ConfigError` so a typo surfaces immediately instead of producing un-callable tool names. Internal logic (token scope filter, read-only write-tool detection, Tasks API augmentation) keys off the canonical bare name regardless of the prefix.
- **`/modules` page replaces the "Report Templates" dashboard real-estate** in the navigation. Reports remain available under MCP Modules -> Zabbix -> Reports - they are not removed, just demoted from the main dashboard surface in favour of the multi-plugin overview.
- **All "Coming in v2.0" wording removed** from sidebar tooltips, page subtitles, and module-row hint text. v1.31 says "Coming soon" instead. Reason: pinning the plugin loader to a specific future version number sets an expectation that may slip; "Coming soon" is honest about it being on the roadmap without locking the schedule.

### Source-tree refactor: bundled Zabbix module moves under `plugins/zabbix/`

The bundled Zabbix integration's source tree moves from `src/zabbix_mcp/` to `plugins/zabbix/zabbix_mcp/`. This puts the bundled module in the same shape future plugins will use, both in the source tree (`plugins/<id>/`) and at runtime (`/opt/zabbix-mcp/plugins/<id>/` once the loader ships). The motivation is to stop the host-vs-plugin asymmetry before it solidifies: every MCP module - bundled or installed - now lives at one canonical path, with its `plugin.json` manifest next to its code, so a future "disable Zabbix and run only third-party plugins" workflow has a clean place to plug in.

**No breaking change for end users.** The pip package name (`zabbix-mcp-server`), the import path (`from zabbix_mcp import ...`), the CLI binary, the systemd unit, the service user, the OAuth issuer URL, the MCP server identity returned in `initialize`, the audit log schema, and existing client connectors are all unchanged. The relocation is internal to the source tree - `pyproject.toml` `[tool.hatch.build.targets.wheel] packages` was updated to `["plugins/zabbix/zabbix_mcp"]`, the wheel build still produces a `zabbix_mcp` top-level package.

A new `[modules.<id>]` config schema is documented for the forthcoming "disable bundled module" capability:

```toml
[modules.zabbix]
enabled = false      # parsed in v1.31, NOT yet enforced (loader release wires it up)
```

v1.31 logs a single info-level message when it sees `enabled = false` so the operator knows the toggle has been seen but not yet enforced; the bundled Zabbix module still registers and exposes its tools to MCP clients. The actual disable / re-enable runtime ships with the plugin loader release (issue #47).

### Plugin readiness (forthcoming, not yet in v1.31)

These items are queued for the next release but listed here so plugin authors know what is coming:

- Plugin **loader runtime** (`install.sh add-plugin / remove-plugin / update-plugin / list-plugins` actually wired up, the corresponding `/modules` UI install / enable / disable / update / remove buttons, the subprocess spawn / forward / lifecycle code). v1.31 ships the discoverability stubs only - the subcommands print a "loader is in development" message and the "+ Install plugin..." button opens a "Coming soon" modal. The actual install path is not yet implemented.
- **Bundled-module disable runtime** - the `[modules.<id>].enabled = false` toggle is parsed and logged in v1.31 but not enforced. The bundled Zabbix module always registers regardless. The disable / re-enable runtime ships with the loader release.
- `--plugin-mode` CLI flag putting the server into stdio plugin mode for use under a future `mcp-extensible` host (so the bundled Zabbix module can run as a plugin under a multi-plugin host instead of standalone).
- `tests/test_plugin_protocol.py` - integration test fixture validating the plugin contract against `examples/example-plugin/` (the example plugin itself already ships in v1.31; the test fixture lands when the loader spawn path lands).

What v1.31 **already ships** (no longer forthcoming): scope grammar (`tool:X`, `plugin:X.read/write`), per-plugin / per-tool consent UI, manifest format + bundled module manifest at `plugins/zabbix/plugin.json`, reference example plugin at `examples/example-plugin/`, placeholder how-to at `docs/PLUGIN-DEVELOPMENT.md`, `[server].tool_prefix` plumbing, install.sh subcommand stubs, /modules admin page + Install plugin modal, NetBox in roadmap, `[plugins.X]` + `[modules.X]` config schema (parsed-not-enforced).

### Upgrade notes

- **No operator action required for v1.30 -> v1.31 upgrade.** No config migration. No filesystem layout change. No systemd unit / service-user change. No audit log schema change. No tools added or removed (the bundled tool list and tool names are byte-for-byte identical to v1.30; `[server].tool_prefix` defaults empty and only namespaces tools when explicitly set). No OAuth client / token / session invalidation. Existing AI clients (Claude Desktop, ChatGPT custom apps, VS Code, JetBrains, ...) keep working without reconfiguration. Run `./deploy/install.sh upgrade` (or `pip install -U zabbix-mcp-server` for pip installs) and restart - that's it.
- **Eager `[plugins.<id>]` blocks are tolerated.** Operators who add a `[plugins.netbox]` (or similar) block to `config.toml` against the documented schema do not crash the v1.31 server - the section is parsed, ignored, and a single info-level startup log line records that plugin entries are not yet active. This is so operators can prepare config alongside the plugin loader release without coordination.
