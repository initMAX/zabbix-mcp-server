...
async def token_create(request: Request) -> Response:
    ...
    if request.method == "GET":
        ...
        # Optional clone-from-existing source: /tokens/create?duplicate_from=<id>
        # pre-fills every field from the source token under a "(copy)"
        # name suffix. Operator adjusts and saves -> brand new token id
        # + fresh raw secret, original token untouched. Useful for
        # spinning up a sibling token (e.g. same scopes, narrower IP
        # range) without retyping every checkbox.
        dup_from = (request.query_params.get("duplicate_from") or "").strip()
        if dup_from:
            src = admin_app.token_store.get_token(dup_from)
            if src is not None:
                ctx["form_name"] = (src.name + " (copy)")[:100]
                ctx["form_scopes"] = ",".join(src.scopes or ["*"])
                ctx["form_allowed_servers"] = ",".join(src.allowed_servers or ["*"])
...
async def token_regenerate(request: Request) -> Response:
    """Issue a fresh raw token for an existing token id.

    Same name, scopes, allowed_servers, allowed_ips, expires_at, and
    read_only flag - just a new secret value. The old raw token stops
    working as soon as the new hash is saved + token_store reloaded,
    so this is the right action when an operator suspects leakage.

    Re-uses tokens/create.html success state to surface the new raw
    value once (the same way creation does), with a `regenerated`
    flag so the page header reads "Token regenerated successfully"
    instead of "Token created successfully".
    """
    admin_app = request.app.state.admin_app
    session = admin_app.require_auth(request)
    if not session or session.role == "viewer":
    ...
    token = admin_app.token_store.get_token(token_id)
    if not token:
        return admin_app.flash_redirect("/tokens", f"Token '{token_id}' not found.", "danger")

    raw_token, token_hash = TokenStore.generate_token()
    try:
        doc = load_config_document(admin_app.config_path)
        tokens_section = doc.get("tokens", {})
        if token_id not in tokens_section:
            return admin_app.flash_redirect("/tokens", f"Token '{token_id}' not in config.", "danger")
        tokens_section[token_id]["token_hash"] = token_hash
        from zabbix_mcp.admin.config_writer import save_config_document
        save_config_document(admin_app.config_path, doc)
        _reload_tokens(admin_app)
        client_ip = request.client.host if request.client else ""
        write_audit("token_regenerate", user=session.user, target_type="token", target_id=token_id, ip=client_ip)
        logger.info("Token '%s' regenerated by %s", token_id, session.user)
        admin_app.restart_needed = True
    except Exception as e:
        logger.error("Failed to regenerate token: %s", e)
        return admin_app.flash_redirect(f"/tokens/{token_id}", f"Failed to regenerate: {e}", "danger")
